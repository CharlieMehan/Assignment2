...
self.Question4();
self.QuestionXYZ(); % new method to answer question
...
view(3)
axis([-1,4,-1,4,0,11]);
trSteps = Lab2Solutionn.trSteps;
for i = 2:length(trSteps)
try delete(findobj(gca, 'Type', 'hgtransform')); 
end % Delete all transforms to delete tranimate's Transform
try delete(text_h); 
end % Delete the text_h
...
disp(num2str(inv(uavTR) * cowHerd.cowModel{cowIndex}.base.T));
%#ok<MINV>
end
cowHerd.PlotSingleRandomStep();
...
robot.qlim
 %% New Method to find UAV's position after specific translations
        function QuestionXYZ()
            % Initial position
            uavTR = eye(4);

            % Define the translational movements
            movements = {
                transl([0, 0, -8]), 
                transl([10, 10, 11]), 
                transl([-5, 3, -2])
            ...
            for i = 1:length(movements)
                % Update the UAV's position after each movement, 
                % the new position is relative to the current position
                uavTR = uavTR * movements{i};
            end

            % Extract the final xyz position from the homogeneous transformation matrix
            finalPosition = uavTR(1:3, 4);
            
            % Display the final position
            disp('The final xyz position of the UAV is:');
            disp(finalPosition');
        end
